...
const Patient = ({ onClose }) => {
  ...
  const confirmDelete = async () => {
    try {
      await fetch(`http://localhost:8000/api/patients/${selectedPatient.id}`, { method: 'DELETE' });
      fetchPatients();
    } catch (err) {
  ...
  return (
    <div className="run-test-overlay">
            ...
            filteredPatients.map(p => (
              <div key={p.id} className="folder-card">
                <div className="folder-icon">
                  <svg width="40" height="20" viewBox="0 0 22 22" fill="none" stroke="#143d3eff" strokeWidth="2">
                  ...
                  <button className="action-btn delete" id="del" onClick={() => handleDeleteClick(p)}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="#e74c3c">
      ...
      {showDeletePrompt && (
        <DeletePrompt onClose={() => setShowDeletePrompt(false)} onConfirm={confirmDelete} patient={selectedPatient} />
      )}
    </div>
  );
};
