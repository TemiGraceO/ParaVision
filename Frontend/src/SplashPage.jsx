...
import BothTest from './BothTest'; // Create this if it doesn't exist yet
...
function SplashPage() {
  ...
  // ?? NEW: Track if splash animation is finished (removes splash forever)
  ...
  useEffect(() => {
    // Only run this animation logic once on mount
      ...
      const intervalId = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setTimeout(() => {
            ...
            // ?? After the fade animation completes, permanently remove splash from DOM
            setTimeout(() => {
              setSplashRemoved(true);
            }, 600); // Wait for splash fade-out transition (0.6s)
          }, 400);
        }
      }, 400);
    }
  ...
  return (
    <>
      {/* -------- SPLASH SCREEN - Renders only until animation completes -------- */}
      {!splashRemoved && (
      ...
      {/* -------- MAIN PAGE (Typewriter) - ALWAYS RENDERED after mount, opacity controlled by CSS -------- */}
      {/* Removed condition {currentView === 'main'} so it persists as background! */}
      <div className='first'>
        <div className='up'>
      ...
      {/* -------- MODALS - Only render after splash is gone -------- */}
      {splashRemoved && showAbout && (
        <div className="about-overlay">
          <About onClose={handleCloseAbout} />
        </div>
      )}

      {splashRemoved && showRunTest && (
      ...
      {splashRemoved && showPatient && (
        <div className="patient-overlay">
          <Patient onClose={handleClosePatientClick} />
        </div>
      )}

      {splashRemoved && showData && (
        <div className="data-overlay">
          <Data onClose={handleCloseData} />
        </div>
      )}

      {splashRemoved && showHistory && (
        <div className="history-overlay">
          <Config onClose={handleCloseHistory} />
        </div>
      )}

      {/* -------- TEST COMPONENTS - Render as overlays on top -------- */}
      {splashRemoved && currentView === 'malaria' && activeTest && (
        <MalariaTest
          patient={activeTest} // ?? FIXED: was testData={activeTest}, but malaria.js expects `patient`
          onClose={handleMalariaTestClose}
        />
      )}
      {/* Add stool and both test components here similarly when you create them */}
 
      {splashRemoved && currentView === 'stool' && activeTest && (
       <Stool
    patient={activeTest} // Stool.js expects `patient`
    onClose={() => {
      setCurrentView('main');
      setActiveTest(null);
    }}
...
{splashRemoved && currentView === 'both' && activeTest && (
  <BothTest
    patient={activeTest} // You can create BothTest.js or reuse both components inside
    onClose={() => {
      setCurrentView('main');
      setActiveTest(null);
    }}
